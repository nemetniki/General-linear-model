...
ktaua = np.linspace(0,4.5,601)
ktaub = np.linspace(0,4.5,601)
Smin = np.zeros((len(ktaua),len(ktaub)))
nus = np.zeros((len(ktaua),len(ktaub)))
SminL = np.zeros((len(ktaua),len(ktaub)))
nusL = np.zeros((len(ktaua),len(ktaub)))
ktauam,ktaubm = np.meshgrid(ktaua,ktaub)

for ia in range(0, len(ktaua)):
    for ib in range(0,len(ktaub)):
        Sout_fb = Squeez(eps*2*np.pi*10,1*np.pi,0,10*2.*pi,ra,ktaua[ia],phia,0,0,10*2.*pi,ra,ktaub[ib],phib,0,0,nu)
        Sout_fbL = Squeez(eps*2*np.pi*10,1*np.pi,0,10*2.*pi,ra,ktaua[ia],phia,5,0,10*2.*pi,ra,ktaub[ib],phib,5,0,nu)
...
np.savetxt("./ktau_map_033eps.txt",np.concatenate((ktauam,ktaubm,nus,Smin,nusL,SminL),axis=0))
